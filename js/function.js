...
//check email address
    ...
    $('.btn-compose-popup').click(function() {
        var target = $(this).attr('href');
    ...
    $('.upload_pp').submit(function() {
        ...
        $(this).ajaxSubmit({
            success:function(data) {
                ...
                $('#editImageModal .div-to-edit').html($('<img />').attr({ id: 'selectArea', 'src': base_url+'mobile/upload/'+data.image }));
    ...
    $('.inspire-text').mouseover(function() {
        var status = $('#inspire-status').val();
        var element = $(this);
        var icon = element.find('i.fa');
        if(status == 1) {
            icon.removeClass('fa-thumbs-up text-success').addClass('fa-thumbs-o-up');
            element.find('span').removeClass('text-success').html('Inspire');
        } else {
            icon.removeClass('fa-thumbs-o-up').addClass('fa-thumbs-up text-success');
            element.find('span').addClass('text-success').html('Inspired');
        }
    });
    $('.inspire-text').mouseout(function() {
        var status = $('#inspire-status').val();
        var element = $(this);
        var icon = element.find('i.fa');
        if(status == 1) {
        ...
        $(this).ajaxSubmit({
            success:function(data) {
                ...
                if(data.msg_code !== undefined && data.msg_code) {
                    ...
                    element.find('.inspire-text').find('span').addClass('text-success').html('Inspired');
                    $('#inspire-status').val(1);
                } else {
                    element.find('i.fa').removeClass('fa-thumbs-up text-success').addClass('fa-thumbs-o-up');
                    element.find('.inspire-text').find('span').removeClass('text-success').html('Inspire');
