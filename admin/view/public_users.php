...
if($a=='list')
{
  $SQL="SELECT users . * , user_details . * FROM  `users`  JOIN user_details ON user_details.id = users.user_detail_id WHERE users.profile_public =  '1'";
...
if($val!="")
{
    $SQL="select user_details.*, users.* from users inner join user_details on users.user_detail_id=user_details.id WHERE users.profile_public =  '1' ";
	if($option==0)
	{
...
$SQL_COUTN="select user_details.*, users.* from users inner join user_details on users.user_detail_id=user_details.id WHERE  users.profile_public =  '1'";
if($val!="")
{
	if($option==0)
	{
...
$thispage=$_SERVER['PHP_SELF']."?p=public_users";
$num=$results1;
$per_page=30;
$showeachside=5;
...
<!-- Content Wrapper. Contains page content -->
	  ...
	  <li><a href="index.php"><i class="fa fa-dashboard"></i>Home</a></li>
            <li><a href="index.php?p=public_users">Users</a></li> 	
          </ol>
		  <?php// echo  $SQL;?>
        </section>
      <div class="content">
                    ...
                    <tbody>
						...
						<tr>
                               ...
                               <!-- <td style="text-align: center;"><?php member_age($res[$i]['birthday']);?></td>-->
                                <td style="text-align: center;"> <?php echo $res[$i]['designation'];?></td>
								<td style="text-align: center;"> <?php echo $res[$i]['username'];?></td>
					   ...
					   <td>
					   <a class="btn btn-success" style="padding:2px;" href="index.php?p=public_users&a=edit&id=<?php echo $res[$i]['user_detail_id']; ?>"><i class="fa fa-edit"></i> &nbsp; Edit</a>
					 <br>
					 <br>
							<?php if($res[$i]['is_deleted']=='1')
								{?>
									<a class="btn btn-danger btn-sm" style="padding:2px;" href="../model/public_users.php?a=block&start=<?php echo $start;?>&is_blocked=0&id=<?php echo $res[$i]['username']; ?>" title="Click to IN-Active Mode"><i class="fa fa-remove"></i>&nbsp; BLOCKED</a>
								<?php }?>
								<?php
								if($res[$i]['is_deleted']=='0')	
								{?>
									<a class="btn btn-warning btn-sm" style="padding:2px;" href="../model/public_users.php?a=block&start=<?php echo $start;?>&is_blocked=1&id=<?php echo $res[$i]['username'];?>" title="Click to Active Mode"><i class="fa fa-hand-pointer-o"></i> &nbsp;Click to block</a>
								<?php }?>
...
{ 
	...
	?>
                ...
                <!-- form start -->
                     <div class="box-body">
                <form class="form-horizontal" method="post" action="../model/public_users.php">
