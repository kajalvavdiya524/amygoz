...
class Controller_Pages extends Controller_Template
{
    ...
    public function before() {
        ...
        $this->template->description = "Callitme is world's first peer-reviewed online network. Join Callitme to review your friends and family and find how trustworthy a person is. With Callitme you can match your single connection, invite someone anonymously for an activity and find interesting people around you.";
    ...
    public function action_index() {
        ...
        $userss = ORM::factory('user')->with('user_detail')
           ...
           $data['viewer_count']=$viewer_count;
            ...
            $data['item']=$userss;
         $this->template->title = "World's first peer-reviewed online network | Callitme";
...
public function action_search_s() { 
    ...
    public function action_search() 
    {
                ...
                $this->template->title = 'Public Search | Callitme';
    ...
    public function action_register() {
        ...
        $this->template->title = 'Join the community of trusted people | Callitme';
    ...
    public function action_signup() {
        ...
        if ($this->request->post()) { // if post request, save user details
            ...
            try {
                ...
                if($age < 13) {
                    Session::instance()->set('error', "We are sorry, but you must be at least 13 years or older to use Callitme. Come back once your hit 13!");
                } else {
    ...
    public function action_newuser_profile() {
        ...
        if($user->registration_steps == 2) {
            ...
            $this->template->title = 'Step 2 | Tell us more about yourself | Callitme';
            $this->template->header = View::factory('templates/header_newuser');
            $this->template->footer ='';// View::factory('templates/footer');
            $this->template->description = "Callitme is world's first peer reviewed network"; //Added by Ash
    ...
    public function action_newuser_photo() {
        ...
        if($user->registration_steps == 3) {
            $this->template = View::factory('templates/pages');
            $this->template->title = 'Step 3 | Upload your profile picture | Callitme';
            $this->template->header = View::factory('templates/header');
            $this->template->footer = '';//View::factory('templates/footer');
    ...
    public function action_newuser_username() {
        ...
        if($user->registration_steps == 4) {
            ...
            $this->template = View::factory('templates/pages');
            $this->template->title = 'Step 4 | Make it easy for others to find you | Callitme';
            $this->template->header = View::factory('templates/header');
            $this->template->footer = '';//View::factory('templates/footer');
            $this->template->description = 'Callitme is your personal network where you can match your single friends,
                review people you know, find singles around you and send requests to your crush anonymously.'; //Added by Ash
    ...
    public function action_newuser_invites() {
        ...
        if($user->registration_steps == 5) {
            ...
            foreach($invites as $invite) {
                if(!$invite->informed) {
                    $this->activity->new_activity('joined', 'just joined Callitme <b> Add as friend</b>.', $invite->user_id, $user->id);

                    $send_email = Email::factory($user->user_detail->get_name()." just joined Callitme! Want to connect?")
            ...
            $this->template = View::factory('templates/pages');
            $this->template->title = 'Step 5 | People that have invited you | Callitme';
            $this->template->header = View::factory('templates/header_newuser');
            $this->template->footer =' ';//View::factory('templates/footer');
            $this->template->description = 'Callitme is your personal network where you can match your single friends,
                review people you know, find singles around you and send requests to your crush anonymously.'; //Added by Ash
    ...
    public function action_login() {
        ...
        $this->template->title = 'Login | Callitme';
    ...
    public function action_logout() {
        ...
        $this->template->title = "Send an Anonymous Request for an Activity | Callitme";
        $this->template->header = View::factory('templates/header', array('page' => 'index')); //template header
        $this->template->content = View::factory('staticpages/signout',$data);
    }

    public function action_thank_you() {
        $this->template->title = 'Callitme | Thank You';
    ...
    public function action_profile() {
        ...
        if (count($data_user)) {
            ...
            $this->template->img = "http://m.callitme.com/upload/" . $user->photo->profile_pic;
    ...
    public function action_forgot_password(){
        ...
        $this->template->title = 'Callitme | Forgot Password';
    ...
    public function action_activate() {
        ...
        if(!empty($email) && !empty($token)) {
            ...
            {   // if token is valid and email address matches activate user and delete the token
                ...
                Session::instance()->set('main_success', 'We welcome you back! Your account has been successfully activated.');
                $this->request->redirect(url::base());
            } else {
                Session::instance()->set('main_error', 'This activation link is invalid or expired. Please visit Callitme website to generate a new one. '); //if link is not valid
            }
    ...
    public function action_reset_password() {
        ...
        if(!empty($email) && !empty($token)) {
            ...
            {   // if token is valid and email address reset user and delete the token
                ...
                $this->template->title = 'Reset Password | Callitme';
    ...
    public function action_pricing() {
        $this->template->title = 'Pricing Option | Callitme';
    ...
    public function action_contact_us() {
        ...
        if($this->request->post()) {
            ...
            if ($validation->check())
            {
                $send_email = Email::factory('Callitme - '.$this->request->post('subject'))
                ->message(View::factory('mails/contact_us_mail', $this->request->post())->render(), 'text/html')
                ->to('support@callitme.com', 'Callitme')
    ...
    public function action_support() {
        $this->template->title = 'Support Questions | Callitme';
 ...
 public function action_resend_link() {
        ...
        $this->template->title = 'Resend Activation Link | Callitme';
        $this->template->content = View::factory('email_form', $data);
    }
    ...
    public function action_directory() {
        ...
        $this->template->title = 'People Directory | Callitme';
     ...
     public function action_directory_public()
        ...
        $data['users'] = $users;
        $data['startswith'] = $startswith;
        $this->template->title = 'Callitme | People Directory';
    ...
    public function action_activity() {
      ...
      public function action_match() {
        $this->template->title = "Match Two Singles | Callitme";
        $this->template->header = View::factory('templates/header', array('page' => 'index')); //template header
    ...
    public function action_localpeople() {
        $this->template->title = "Find Local People Around You | Callitme";
        $this->template->header = View::factory('templates/header', array('page' => 'index')); //template header
    ...
    public function action_localdating() {
        $this->template->title = "Find Local Singles | Callitme";
        $this->template->header = View::factory('templates/header', array('page' => 'index')); //template header
        $this->template->content = View::factory('staticpages/localdating');
    }

    public function action_import() {
        $this->template->title = "See Who You Know | Callitme";
        $this->template->header = View::factory('templates/header', array('page' => 'index')); //template header
  ...
  public function action_matchme() 
   ...
   public function action_search_results()
       ...
       // $this->auto_render = false;
            ...
            if ($this->request->post()) 
            {
                ...
                $user = ORM::factory('user')->with('user_detail')
                   ...
                   $this->template->title = 'Search Results | Callitme';
    ...
    public function action_invalid_search()
        ...
        {
                Session::instance()->set('error', 'This person is not an active  ');
                $this->template->title = "See Who You Know | Callitme";
                $this->template->header = View::factory('templates/header', array('page' => 'index')); //template header
                $this->template->content = View::factory('staticpages/invalid_search');
