...
class Controller_Pp extends Controller_Template
{
    ...
    public function before() {
        ...
        $this->template->description = "Find, meet, connect and get inspired by friends on Amygoz";
    ...
    public function action_index() {
        ...
        if($user->profile_public == '1' && $user->is_deleted == 0) {
            ...
            $keyword .= "Public opinion on ".$name.",";
            ...
            if(count($data_user)) {
                ...
                $this->template->img = "https://m.amygoz.com/upload/" . $user->photo->profile_pic_s;
    ...
    public function action_write_review() {
        ...
        if($this->request->post()) {
            ...
            if(!empty($words)) {
                ...
                if($word_exists) { // return with an error.
                    Session::instance()->set('error_pp', 'This review contains some improper words that are not allowed.');
        ...
        if($password == 'Yes') {
            ...
            if (Auth::instance()->login($username, $password, true)) { // if login successfull
                ...
                Session::instance()->set('error_pp', 'Wrong Username and Password .');
            }
            ...
            if($this->request->post()) {
                ...
                Session::instance()->set('error_pp', 'Your Review is not submitted successfully.');
                $this->request->redirect(url::base()."public/".$user_to->username);
            }
        } else {
            if ($this->request->post()) { // if post request, save user details
                ...
                try {
                    ...
                    if($age < 13) {
                        ...
                        $post_data1['message']=$post_data['message'];
    ...
    public function action_create() {
        if(Auth::instance()->logged_in()) { 
            ...
            $this->template->keywords ="Create public personality profile, Create Callitme Public Profile";
            $this->template->description ="Create a Public Personality in Callitme";
            $this->template->title = "Create a Public Personality Profile | Callitme";
    ...
    public function action_review() {
        ...
        if($user->profile_public == '1' && $user->is_deleted == 0) {
            ...
            $keyword.="Public opinion on ".$name.",";
            $keyword.=" ".$name." ".$user->user_detail->location.", ";
            if(!empty($user->user_detail->designation)){ $keyword.=$user->user_detail->first_name." ".$user->user_detail->last_name ." ".$user->user_detail->designation.', ' ; }
            if(!empty($user->user_detail->employment)){ $keyword.= " at " .$user->user_detail->employment. ", " ; }
            if(!empty($data['social'])){ $keyword.=  "Amygoz profile with social percentile " . $data['social']. "%, " ; }
            if(!empty($user->friends)){ $keyword.=  "Friends of ".$user->friends."".$name. "," ; }

            if (count($data_user)) {
                $this->template->keywords=  $keyword;
                $this->template->title = $user->user_detail->first_name . ' ' . $user->user_detail->last_name ." is at ". $data['social'].'%'. ' | Write your review here to increase '.$user->user_detail->first_name."'s"." score";
                $this->template->img = "https://www.amygoz.com/mobile/upload/" . $user->photo->profile_pic_s;
                $this->template->description = 'Read what people say about ' . $user->user_detail->first_name . ' ' . $user->user_detail->last_name .' the '. $user->user_detail->designation .' and write your own opinion.';
                //$this->template->header = View::factory('templates/header', array('page' => 'index')); //template header
                $this->template->content = View::factory('pp/review', $data);
            } else {
                $this->request->redirect(url::base()."invalid_search");
            }
        } else  {
