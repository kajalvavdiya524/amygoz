...
class Controller_Peoplereview extends Controller_Template {
    ...
    public function action_send() {
        ...
        if($this->request->post()) {
            ...
            Session::instance()->set('open_box', true);
            ...
            if(!$this->request->post('edit_recommend') && ORM::factory('recommend')->where('to', '=', $user_to->id)->where('from', '=', $user->id)->count_all()) {
                ...
                Session::instance()->set('open_box', false);
    ...
    public function action_send_annonymaous() {
        ...
        if($this->request->post()) {
            ...
            if($user->email === $user_to->user->email)
            {
                Session::instance()->set('error', 'You can\'t review yourself.');
                $this->request->redirect(url::base()."peoplereview/compose");
            }
            if(!$this->request->post('edit_recommend') && ORM::factory('recommend')->where('to', '=', $user_to->user->id)->where('from', '=', $user->id)->count_all())
            {
                $recommend = ORM::factory('recommend', $this->request->post('edit_recommend'));
                $already = ORM::factory('recommend')->where('to', '=', $user_to->user->id)->where('from', '=', $user->id)->find();
                $msg = "You have already reviewed ".$user_to->first_name .".
                You can <a href=\"".url::base()."peoplereview/compose?id=".$already->id."\">Edit the review</a> you wrote for ".$user_to->first_name;
                $this->activity = new Model_Activity;
                $this->activity->new_s_activity('anon_edit_recommend', ' has reviewed you', $recommend->id, $user_to->user->id);
                Session::instance()->set('error', $msg);
                $this->request->redirect(url::base()."peoplereview/compose");
            }
            else
            {
                ...
                $recommend->values($post_data);
                $recommend->save();
                if($user->rec_req->where('from', '=', $user_to->user->id)->count_all()) 
                {
                    $user->rec_req->where('from', '=', $user_to->user->id)->find()->delete();
                }
                $this->activity = new Model_Activity;
                $this->activity->new_s_activity('anon_recommend', ' has reviewed you', $recommend->id, $user_to->user->id);
                     if($user_to->rec_alert)
                     {
                         //send email
                         $from = Auth::instance()->get_user();
                         $send_email = Email::factory('Review from  Anonymous user')
                                     ->message(View::factory('mails/new_rec_anon', array('user_to' => $user_to, 'message' => $recommend->message))->render(), 'text/html')
        ...
        if($this->request->post()) {
            ...
            if($user->email === $this->request->post('email')) {
                ...
                if(ORM::factory('recommend')->where('to', '=', $user->id)->where('from', '=', $user_to->id)->count_all()) {
                    $msg = $user_to->user_detail->get_name() ." has already reviewed you once. 
                    ...
                    if($user_to->user_detail->rec_alert) {
                        ...
                        $send_email = Email::factory('Request to Review from '.$from->user_detail->first_name .' '.$from->user_detail->last_name)
                            ->message(View::factory('mails/recommend_request_mail', array('to' => $user_to->user_detail))->render(), 'text/html')
                            ->to($user_to->email)
                            ->from('noreply@callitme.com', 'Callitme')
                            ->send();
                    }
