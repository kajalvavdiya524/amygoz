...
class Controller_Activity extends Controller_Template {
    ...
    public function action_send22july2017() 
    {
        $user = Auth::instance()->get_user();
        if($this->request->post()) {
            ...
            if($user->user_detail->sex === $user_to->sex) {
            ...
            if(count($s_friends) >= 7) {
                ...
                if(!$user_to->user->id || !$user->check_friends($user)) {
                    ...
                    if($user->plan->r_to_anyone_used == $user->plan->r_to_anyone) {
                        ...
                        $redirect_url = ($user_to->user->id) ? 'activity?user='.$user_to->user->username : 'activity';
                ...
                shuffle($arequest_people);
                $arequest_data['to'] = $user_to->user->id;
                $arequest_data['from'] = $user->id;
                ...
                $this->activity = new Model_Activity;
                $this->activity->new_activity('arequest', ' You have a new '.$this->request->post('plan').' request.', $arequest_id, $user_to->user->id);

                if($user_to->req_alert) 
                {
                    //send email
