    ...
    public function action_add_post() {
        ...
        if($_FILES["picture"]["size"] > 0) {
            ...
            if(strtolower($ext) == 'gif') {
                ...
                $content['photo'] = url::base().'upload/'.$gif_name;
            }
            else {
                $content['photo'] = url::base().'upload/'.$name;
            }
            //$content['photo'] = url::base().'upload/'.$name;

            $this->post = new Model_Post;
            $post = $this->post->new_post_with_image('post', $content);
            
            $this->activity = new Model_Activity;
            $this->activity->new_activity('new_photo', 'uploaded a new photo', $post->id, null);
 ...
 /*  public function action_search() { 
    ...
    */
    ...
    public function action_profile() {
        ...
        $this->template->img = url::base()."upload/".$user->photo->profile_pic_s;
...
public function action_askphoto()
    ...
    public function action_search_member() {
        $this->template = View::factory('templates/v2/profile');
        $this->template->header  = View::factory('templates/v2/member-header');
    ...
    public function action_inspire() {
        ...
        if($this->request->post('user_id')) {
            ...
            if($inspire->status) {
                ...
                $post = $this->post->new_post_with_image('inspired', array('photo' => url::base().'upload/'.$user->photo->profile_pic_o), " is inspired by @".$user->username ." .");
            } else {
                $post = $this->post->new_post_with_image('inspired', array('photo' => url::base().'upload/'.$user->photo->profile_pic_o), " is not inspired by @".$user->username ." anymore.");
            }
