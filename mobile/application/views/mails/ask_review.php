     ...
     <table class="sprty brd-pag-2">
      <tr>
       ...
       <table class="sprty">
          <tr>
            <td>                                               
          <p style="color:#888; font-size:17px;padding: 2px 25px 0px 30px;">                                                        
  ...
  <table class="sprty">
       ...
       <span style="padding-left:10px;background-color:#bdbcbc;border-radius:50%;padding:16px;font-size:21px;margin:0px auto;text-align:center;float: left;">
        ...
        <?php } ?>
         ...
         <a style="font:16px/25px Arial, Helvetica, sans-serif;color:#888;text-decoration: none;" href="<?php echo url::base() . "pages/redirect_to/" ."?page=" . urlencode($from->username); ?>"><?php echo $from->user_detail->first_name." ".$from->user_detail->last_name; ?>
            ...
            <br/>                                                   
        <?php $recommendations = $user->recommendations->where('state', '=', 'approve')->order_by('time', 'desc')->find_all()->as_array();
        $temp_words = array();
        ...
        $percentile_s= $user->friends->calculate_social_percentage($tags);
        ?>
        Your Social Percentile is <?php echo $percentile_s;?>%  
            </p>
          </li>
        </ul>
      </td>
    ...
    <tr>
      <td class="row">
        ...
        $photo_image1 = file_exists("upload/".$photo);
        if(!empty($photo)&& $photo_image) { ?>
        <img alt="" src="<?php echo url::base().'mobile/upload/'.$friend->photo->profile_pic;?>" style="width: 50px;height: 50px;border-radius: 50%;float: left;">
        <?php } 
        else if(!empty($photo)&& $photo_image1) { ?>
        <img alt="" src="<?php echo url::base().'upload/'.$friend->photo->profile_pic;?>" style="width: 50px;height: 50px;border-radius: 50%;float: left;">
        <?php } else { ?>
        <span style="padding-left:10px;background-color:#bdbcbc;border-radius:50%;padding:16px;font-size:21px;margin:0px auto;text-align:center;float: left;">
    ...
    <table>
        <tr>
          <td>
          <p style="color:#888; font-size:15px;padding: 10px 25px 0px 30px;">
         <a style="font-size:15px;color:#00bcd4;text-decoration: none;" href="#">Profiles with a higher Social Percentile and good Reviews are more trusted than the ones with fewer reviews. Increase your credibility by getting a review from <?php echo $friend->user_detail->get_name(); ?>
        </a> 
      </p>
     </td>
    </tr>                                                               
     ...
     <tr style="text-align: center;">
        ...
        Invite people you know to write a review for you 
        <a href="<?php echo url::base()."pages/redirect_to/".$user->username ."?page=import"; ?>" class="btn btn-primay btn-sm" style="text-decoration:none; color:#fff;background: #00bcd4;padding: 9px 35px; "> Invite Here
        </a>
       </p>
      </td>
     </tr>
