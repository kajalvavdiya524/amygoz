...
@media only screen and (max-width: 768px) {
    ...
    <body>
      ...
      <tr>
        ...
        <img alt="" src="<?php echo url::base().'mobile/upload/'.$newemail->photo->profile_pic;?>" style="width: 35px;height: 35px;border-radius: 50%;padding: 0px 2px 0px 0px;">
        <?php } 
        else if(!empty($photo)&& $photo_image2) { ?>
        <img alt="" src="<?php echo url::base().'upload/'.$newemail->photo->profile_pic;?>" style="width: 35px;height: 35px;border-radius: 50%;padding: 0px 2px 0px 0px;">
        <?php } else { ?>
        <span style="background-color:#bdbcbc;border-radius:50%;padding:8px;font-size:20px;margin:0px auto;text-align:center;">
        <?php echo ucwords($newemail->user_detail->first_name[0])."".ucwords($newemail->user_detail->last_name[0]);?>
         </span>
        <?php } ?>
                </td>
              </tr>
        ...
        Hi <?php echo $newemail->user_detail->first_name;?>,
        </p>
    </td>
    </tr>
        <tr>
              <td>                                               
          <p style="font-size:17px;color:#888;padding: 2px 25px 20px 30px;">
        <?php echo ucwords($from->user_detail->first_name." ".$from->user_detail->last_name);?> has uploaded a new photo on Callitme.
        </td>
    </tr>
         </table>
...
<table class="sprty">
    <tr>
      <td class="row">
        ...
        Social % <?php echo $from->calculate_social_percentage($tags);?>
           ...
           <table class="sprty">
            <tr>
            <td>
        <p style="color:#888; font-size:15px; padding: 20px 25px 0px 30px;">                                                                
        As you requested, I have uploaded my profile picture. Please check it out. I hope you like it.
          </p>
          </td>
          </tr>
          </table>                                                              
     ...
     <table class="sprty" style="padding: 30px 0px 35px 30px;">
        ...
        <a target="_blank" style="background: #00bcd4;color:#fff;padding: 9px 30px;text-decoration:none;" href="<?php echo url::base()."pages/redirect_to/$username". "?page=".urlencode($username);?>">
        Check it out
        </a>
         </td>
         </tr>
