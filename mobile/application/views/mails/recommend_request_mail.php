        ...
        <tr>
            <td>
                        ...
                        <td style="/*! text-align: right; */width: 49px;vertical-align: middle;">
                            <?php 
                                ...
                                if(!empty($photo)&& $photo_image1) { ?>
                                    <img alt="" src="<?php echo url::base().'mobile/upload/'.$to->user->photo->profile_pic;?>" style="width: 35px;height: 35px;border-radius: 50%;padding: 0px 2px 0px 0px;">
                                <?php } 
                                else if(!empty($photo)&& $photo_image2) { ?>
                                    <img alt="" src="<?php echo url::base().'upload/'.$to->user->photo->profile_pic;?>" style="width: 35px;height: 35px;border-radius: 50%;padding: 0px 2px 0px 0px;">
                                <?php } else { ?>
                                <span style="background-color:#bdbcbc;border-radius:50%;padding:8px;float:left;font-size:20px;font-weight:600;"> 
                                    <?php echo $to->get_no_image_name();?>
                                </span>
                            <?php } ?>
                        </td>
                    </tr>
                ...
                <table class="sprty">
                    ...
                    <tr>
                        <td>
                            <p style="color:#888; font-size:15px;padding: 20px 25px 12px 30px;">                                           
...
<table class="sprty">
    <tr>
      <td class="row">
        ...
        <img alt="" class="CToWUd" src="<?php echo url::base().'mobile/upload/'.$from->photo->profile_pic;?>" style="width: 50px;height: 50px;border-radius: 50%;float: left;">
        <?php } 
        else if(!empty($photo)&& $photo_image1) { ?>
        <img alt="" class="CToWUd" src="<?php echo url::base().'upload/'.$from->photo->profile_pic;?>" style="width: 50px;height: 50px;border-radius: 50%;float: left;">
        <?php } else { ?>
        ...
        <a style="font:16px/25px Arial, Helvetica, sans-serif;color:#888;text-decoration: none;" href="<?php echo url::base() . "pages/redirect_to/" ."?page=" . urlencode($from->username); ?>"><?php echo $from->user_detail->first_name." ".$from->user_detail->last_name; ?>
        </a>
       </span>
            <br/>  
    ...
    <table class="sprty">
        <tr>
          <td>
          <p style="color:#888; font-size:15px;padding: 25px 25px 12px 30px;">                                                         
        Could you do me a favor by 
        <a href="<?php echo url::base()."pages/redirect_to/".$to->user->username ."?page=".urlencode("peoplereview/recommend_request"); ?>" style="color: #00bcd4;text-decoration:none;">
        reviewing me </a> in Callitme? Your review about me would affect my Social Percentile.
        </p>
        </td>
        </tr>
        </table>  
        ...
        <tr>
          <td>
          <p style="color:#888; font-size:15px;padding: 15px 25px 12px 30px;">                                                              
        Please <a href="<?php echo url::base()."pages/redirect_to/".$to->user->username ."?page=".urlencode("peoplereview/recommend_request"); ?>" style="color: #00bcd4;text-decoration:none;">
        click here to review me.</a>
        </p>
        </td>
        </tr>
        </table>
