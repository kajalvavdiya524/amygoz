...
class Controller_Messageapi extends Controller_Template 
{
    ...
    public function action_show_conversation_user_list() 
    {
        ...
        $session_user = ORM::factory('user', array('username'=>$this->request->post('username')));
        ...
        foreach($messages as $message )
        {
            ...
            $conversation_member[]=array('Name'=>$Name,'time'=>$time,'message'=>$message,'username'=>$username);        
        }
    ...
    public function action_compose() 
    {
        ...
        {    
            ...
            if($validation->check())
            {
                ...
                if($this->request->post('message') && $this->request->post('username')) 
                {
                    ...
                    if($proceed) 
                    {
                        ...
                        $from = ORM::factory('user',array('username'=>$this->request->post('username')))->user_detail;
                        ...
                        if($user_to->user_detail->msg_alert) 
                        {
                            ...
                            ->message(View::factory('mails/new_message_mail_api', $mail_data)->render(), 'text/html')
                            ->to($user_to->email)
        ...
        if($validation->check())
        {
            if($this->request->post('reply')) 
            {
                ...
                if($user_to->user_detail->msg_alert) {
                    ...
                    //send email
                    //$from = ORM::factory('user',array('username'=>$this->request->post('username')));
                    $send_email = Email::factory('Message from '.$from->first_name .' '.$from->last_name)
                    ->message(View::factory('mails/new_message_mail_api', $mail_data)->render(), 'text/html')
                    ->to($user_to->email)
                    ->from('noreply@Callitme.com', 'Callitme')
        ...
        if($this->request->query())
        {
            ...
            if($validation->check())
            {
                ...
                foreach(array_reverse($conversatios) as $conversation) 
                {
                    ...
                    $profile_pic = $conversation->owner->photo->profile_pic_s;
