...
class Controller_Pages extends Controller_Template
{
    ...
    public function before() {
        ...
        $this->template->description = "Get inspired by great people around you.";
    ...
    public function action_register() {
        ...
        $this->template->title = 'Join the community of trusted people | Amygoz';
    ...
    /*public function get_username_select($birthday,$first_name,$last_name)
     {
     $suggestions = array();
    ...
    public function action_newuser_profile() {
        ...
        if($user->registration_steps == 2) {
            ...
            $this->template->description = "Amygoz is world's first peer reviewed network"; //Added by Ash
    ...
    public function action_newuser_photo() {
        ...
        if($user->registration_steps == 3) {
            ...
            $this->template->header = View::factory('templates/header');
            $this->template->footer = '';//View::factory('templates/footer');
            $this->template->description = 'Amygoz is your personal network where you can match your single friends,
    ...
    public function action_save_photo() {
        ...
        $upload_folder = DOCROOT."mobile/upload/";
        if( $this->request->post() ) {
            $user = Auth::instance()->get_user();
            ...
            if( $this->request->post('crop') ) {
                ...
                $original = $upload_folder.$this->request->post('imag_name');
            ...
            {
                $picture = Upload::save($_FILES['picture'], null , $upload_folder);
                $str = Text::random();
    ...
    public function action_newuser_username() {
        ...
        if($user->registration_steps == 4) {
            ...
            $this->template = View::factory('templates/pages');
            $this->template->title = 'Step 4 | Make it easy for others to find you | Amygoz';
            $this->template->header = View::factory('templates/header');
            $this->template->footer = '';//View::factory('templates/footer');
            $this->template->description = 'Amygoz is your personal network where you can match your single friends,
                review people you know, find singles around you and send requests to your crush anonymously.'; //Added by Ash
    ...
    public function action_profile() {
        ...
        if (count($data_user)) {
            ...
            $this->template->img = "http://www.amygoz.com/mobile/upload/" . $user->photo->profile_pic;

            $this->template->description = 'View ' . $name . '\'s full profile on Amygoz. Amygoz helps people like you and ' . $name . ' from ' .$user->user_detail->location . ' find new friends and meet trusted people online. Find ' . $name . '\'s  friends, family, education, school, job and reviews.';
            $this->template->header = View::factory('templates/header', array('page' => 'index')); //template header
    ...
    public function action_forgot_password(){
        ...
        $this->template->title = 'Amygoz | Forgot Password';
    ...
    public function action_pricing() {
        $this->template->title = 'Pricing Option | Amygoz';
        ...
        if($this->request->post()) {
            ...
            if ($validation->check())
            {
                $send_email = Email::factory('Amygoz - '.$this->request->post('subject'))
                    ...
                    ->to('support@callitme.com', 'Amygoz')
    ...
    public function action_activity() {
        $this->template->title = "Send an Anonymous Request for an Activity | Amygoz";
        $this->template->header = View::factory('templates/header', array('page' => 'index')); //template header
    ...
    public function action_invalid_search()
    {
        ...
        $this->template->title = "See Who You Know | Amygoz";
        $this->template->header = View::factory('templates/header', array('page' => 'index')); //template header
    ...
    public function action_unsubscribe() {
        $param = $this->request->query('token');

        $encrypt = Encrypt::instance('tripledes');
        $str = ($param);
        $string = $encrypt->decode($str);

        $email = '';
        $from = '';
        $attr = explode(' from ', $string);
        ...
        if(!empty($email) && !empty($from)) {
            //find user from username
            $user = ORM::factory('user', array('email' => $email));
            $from = ORM::factory('user', array('email' => $from));

            if($user->id && $from->id) {
                $exists = ORM::factory('unsubscribe')
                    ->where('unsubscribe_user_id', '=', $user->id)
                    ->where('specific_user_id', '=', $from->id)
                    ->where('type', '=', 'invite_mail')
                    ->count_all();

                if(!$exists) {
                    $data = array();
                    $data['unsubscribe_email'] = $user->email;
                    $data['unsubscribe_user_id'] = $user->id;
                    $data['specific_user_id'] = $from->id;
                    $data['type'] = 'invite_mail';
                    $data['created'] = date('Y-m-d H:i:s');

                    $unsubscribe = ORM::factory('unsubscribe');
                    $unsubscribe->values($data);
                    $unsubscribe->save();
                }

                $query = DB::update('invites')
                    ->set(array('email_notification' => '1'))
                    ->where('invitee_id', '=', $user->id)
    ...
    public function action_post_detail() {
        ...
        if(isset($post->id)) {
            $this->template->header = View::factory('templates/header', array('page' => 'index'));
