...
class Model_Invite extends ORM {
    ...
    public function send_invite($user, $user_to, $tags, $mail = true) {
        ...
        if($mail) {
            ...
            $str = $user_to->email.' from '.$user->email;
            
            $encrypt = Encrypt::instance('tripledes');
            $str = $encrypt->encode($str);

            $data['unsubsribe_url'] = url::base().'pages/unsubscribe?token='.urlencode($str);
            ...
            $send_email = Email::factory($user->user_detail->get_name()." wants you to join Callitme")
                ...
                ->to($user_to->email)
                ->from('noreply@callitme.com', $user->user_detail->get_name() .' via Callitme')
                ->send();
        }
    }
