      ...
      <tr>
        <td>
          <table class="header" style="background-color:rgb(0, 188, 212);font-size: 12px;">
            <tr class="header">
              <td>
        <a target="_blank" href="https://www.callitme.com/" style="position: relative;top:0px;height: 37px;">
        <img src="https://m.callitme.com/img/logo1.png" alt="CALLITME.COM" style="position: relative;top:0px;height: 37px;padding: 0px 0px 0px 25px;"/>
         ...
         <td style="text-align:right;padding-right:5px;vertical-align: middle;"> 
          <p class="park-tf">                                                                                                           
        <?php echo ucwords($match->match_by->user_detail->first_name.' '.$match->match_by->user_detail->last_name);?>
          </p>
          </td>                                                                                                                   
          <td style="width: 49px;vertical-align: middle;">                                        
        <?php 
        $photo = $match->match_by->photo->profile_pic;
        $photo_image1 = file_exists("mobile/upload/".$photo);
        $photo_image2 = file_exists("upload/".$photo);
        if(!empty($photo)&& $photo_image1) { ?>
        <img alt="" src="<?php echo url::base().'mobile/upload/'.$match->match_by->photo->profile_pic;?>" style="width: 35px;height: 35px;border-radius: 50%;padding: 0px 2px 0px 0px;">
        <?php }
        else if(!empty($photo)&& $photo_image2) { ?>
        <img alt="" src="<?php echo url::base().'upload/'.$match->match_by->photo->profile_pic;?>" style="width: 35px;height: 35px;border-radius: 50%;padding: 0px 2px 0px 0px;">
        <?php }  else { ?>
        <span style="background-color:#bdbcbc;border-radius:50%;padding:8px;font-size:20px;margin:0px auto;text-align:center;">
        <?php echo ucwords($match->match_by->user_detail->first_name[0])."".ucwords($match->match_by->user_detail->last_name[0]);?>
        </span>                                                                      
        <?php } ?>
         </td>
              </tr>
         ...
         <strong>Congratulations! </strong> <?php echo $match->match_by->user_detail->first_name; ?>
           </p>
            </td>
          </tr>
        ...
        <tr>
          <td>
            <p style="color:#888; font-size:15px;padding: 15px 25px 25px 30px;">                                             
        <strong> You matched</strong> <?php echo $match->match_with->user_detail->first_name; ?> and <?php echo $match->match_to->user_detail->first_name; ?>
            </p>
            </td>
          </tr>                                          
      </table> 
...
<table class="sprty">
    <tr>
      <td class="row">                                       
        ...
        <a href="<?php echo url::base() . "pages/redirect_to/" ."?page=" . urlencode($match->match_with->username); ?>">  
        <?php 
        $photo =  $match->match_with->photo->profile_pic;
        $photo_image = file_exists("mobile/upload/".$photo);
        $photo_image1 = file_exists("upload/".$photo);
        if(!empty($photo)&& $photo_image) { ?>
        <img alt="" src="<?php echo url::base().'mobile/upload/'. $match->match_with->photo->profile_pic;?>" style="width: 50px;height: 50px;border-radius: 50%;">
        <?php } 
         else if(!empty($photo)&& $photo_image1) { ?>
        <img alt="" src="<?php echo url::base().'upload/'. $match->match_with->photo->profile_pic;?>" style="width: 50px;height: 50px;border-radius: 50%;">
        <?php } else { ?>
        <span style="padding-left:10px;background-color:#bdbcbc;border-radius:50%;padding:16px;font-size:21px;margin:0px auto;text-align:center;">
        <?php echo $match->match_with->user_detail->first_name[0]."". $match->match_with->user_detail->last_name[0];?>
        </span> 
        <?php } ?>
        </a>
        <br/>
         <p style="font-size: 15px;color: black;font-weight: 500;">
        <?php echo $match->match_with->user_detail->first_name. ' '.$match->match_with->user_detail->last_name; ?>
        </p>
        </li>
        <li class="unline-list" style="list-style-type: none;padding: 0px 6px 0px 0px;">
        <img src="<?php echo url::base().'img/shake.jpg';?>" style="width: 90px; height: 70px;">
        </li>
        
        <li class="unline-list" style="list-style-type: none;padding: 0px 6px 0px 28px;">                                            
        <a href="<?php echo url::base() . "pages/redirect_to/" ."?page=" . urlencode($match->match_to->username); ?>">  
        <?php 
        $photo =  $match->match_to->photo->profile_pic;
        $photo_image = file_exists("mobile/upload/".$photo);
        $photo_image1 = file_exists("upload/".$photo);
        if(!empty($photo)&& $photo_image) { ?>
        <img alt="" src="<?php echo url::base().'mobile/upload/'. $match->match_to->photo->profile_pic;?>" style="width: 50px;height: 50px;border-radius: 50%;">
        <?php } 
        else if(!empty($photo)&& $photo_image1) { ?>
        <img alt="" src="<?php echo url::base().'upload/'. $match->match_to->photo->profile_pic;?>" style="width: 50px;height: 50px;border-radius: 50%;">
        <?php } else { ?>
        <span style="padding-left:10px;background-color:#bdbcbc;border-radius:50%;padding:16px;font-size:21px;margin:0px auto;text-align:center;">
        <?php echo $match->match_to->user_detail->first_name[0]."". $match->match_to->user_detail->last_name[0];?>
        </span> 
        <?php } ?>
         </a>
         <br/>
         <p style="font-size: 15px;color: black;font-weight: 500">
          <?php echo $match->match_to->user_detail->first_name." ". $match->match_to->user_detail->last_name;?>
          </p>
        </li>
         </ul>
         </td>
