...
@media only screen and (max-width: 768px) {
  ...
  <!--   <table class="sprty brd-pag-2">
      <tr>
        <td>
          <table class="header" font-size: 12px;>
            <tr class="header">
              <td>  
      ...
      <td style="/*! text-align: right; */width: 49px;vertical-align: middle;">                                                   
        ...
        <table class="sprty">
            <tr>
              <td>                                               
          <p style="font-size:17px;color:#888;padding: 10px 25px 0px 30px;">           
              Hi <?php echo $to->user_detail->first_name;?>,<br/><br/>
              <span style="color:#888; font-size:13px;padding: 0px 25px 4px 30px;">                                            
                <a href="<?php echo url::base()."pages/redirect_to/".$from->username ."?page=".urlencode($from->username); ?>"><?php echo $from->user_detail->first_name .' '.$from->user_detail->last_name?>
                </a> wants to connect with you in Callitme. 
              </span>
            </p>
            </td>
          </tr>
   ...
   <table class="sprty">
    ...
    <a href="<?php echo url::base()."pages/redirect_to/".$from->username ."?page=".urlencode($from->username); ?>">  
        <?php 
           $photo = $from->photo->profile_pic;
         $photo_image = file_exists("mobile/upload/".$photo);
          $photo_image1 = file_exists("upload/".$photo);
       if(!empty($photo)&& $photo_image) { ?>
           <img alt="<?php echo $from->user_detail->first_name." ".$from->user_detail->last_name;?>" src="<?php echo url::base().'mobile/upload/'.$from->photo->profile_pic;?>" class="img-rect posh-ytr" style="width: 50px;height: 50px;border-radius: 50%;float: left;">
         <?php } 
            else if(!empty($photo)&& $photo_image1) { ?>
               <img alt="<?php echo $from->user_detail->first_name." ".$from->user_detail->last_name;?>" src="<?php echo url::base().'upload/'.$from->photo->profile_pic;?>" class="img-rect posh-ytr" style="width: 50px;height: 50px;border-radius: 50%;float: left;">
           <?php } else { ?>
      ...
      <!--Review and message buttons begin-->             
<!--       <table class="sprty" style="padding: 30px 0px 35px 30px;">
        <tr class="row">
          <td class="col-xs-12" style="text-align: center;padding: 0px 0px 0px 25px;">                       
               <a target="_blank" style="background: #00bcd4;color:#fff;padding: 9px 30px;text-decoration:none;" href="<?php echo url::base()."pages/redirect_to/".$from->username ."?page=".urlencode("peoplereview/compose?ask=".$from->username); ?>">Review</a>
              </td>                    
       <td class="col-xs-6" style="text-align: left;padding: 0px 25px 35px 0px;">             
               <a target="_blank" style="background:#ff2a7f;text-decoration:none; color:#fff;padding:9px 30px;" href="<?php echo url::base()."pages/redirect_to/".$from->username ."?page=".urlencode("chat/compose?user=".$from->username); ?>">Message</a>
                </td>
            </tr>
        </table>   -->
        <!--Review and message buttons end-->  
