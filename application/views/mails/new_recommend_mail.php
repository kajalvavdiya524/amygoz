    ...
    <table class="sprty brd-pag-2">
      <tr>
       ...
       <td style="text-align:right;padding-right:5px;vertical-align: middle;"> 
        ...
        if($user_to->user->photo->profile_pic) { ?>
        <img alt="" src="https://m.callitme.com/upload/<?php echo $user_to->user->photo->profile_pic;?>" style="width: 35px;height: 35px;border-radius: 50%;padding: 0px 2px 0px 0px;">
        <?php }  else { ?>
      ...
      <table class="sprty">
        ...
        Hi <?php echo $user_to->first_name;?>,
           </p>
           </td>
           </tr> 
           ...
           <tr>
            <td>                                               
          <p style="color:#888; font-size:15px;padding: 4px 25px 0px 30px;">                                           
        <strong>Congratulations!</strong> 
        <a href="<?php echo url::base()."pages/redirect_to/".$user_to->user->username ."?page=".urlencode($from->username); ?>" style="color: #01BF01;text-decoration:none;">
        <?php echo $from->user_detail->first_name." ".$from->user_detail->last_name; ?>
        </a> has reviewed you in Callitme.
        </p>
        </td>
        </tr>
        </table>
 ...
 <table class="sprty">
    <tr>
      <td class="row">
        ...
        $photo_image1 = file_exists("upload/".$photo);
        if($from->photo->profile_pic) { ?>
        <img alt="" src="https://m.callitme.com/upload/<?php echo $from->photo->profile_pic;?>" style="width: 50px;height: 50px;border-radius: 50%;float: left;">
        <?php } else { ?>
        ...
        <a style="font-size:15px;color:#888;text-decoration: none;" href="<?php echo url::base() . "pages/redirect_to/" ."?page=" . urlencode($from->username); ?>"><?php echo $from->user_detail->first_name." ".$from->user_detail->last_name; ?>
        </a>
         </span>
            <br/>                                                               
    ...
    <table>
        <tr>
          <td>
            <p style="color:#888; font-size:15px;font-weight:600;padding: 8px 25px 0px 30px;">                                                              
       ...
       <tr>
          <td>
            <p style="color:#888; font-size:15px;padding: 8px 25px 0px 30px;">                                             
        Please log in to your <a href="<?php echo url::base()."pages/redirect_to/".$user_to->user->username ."?page=".urlencode("peoplereview"); ?>" style="color: #01BF01;text-decoration:none;"> Callitme </a> account to 
        <u>Show</u> or <u>Hide</u> this review from your profile. Please note that if you don’t either show or hide this review from your profile, it will automatically be published on your profile in few days.
        </p>
        </td>
        </tr> 
         ...
         <tr>
          <td>
            <p style="color:#888; font-size:15px;padding: 8px 25px 35px 30px;">                                               
        Return the favor by <a href="<?php echo url::base()."pages/redirect_to/".$user_to->user->username ."?page=".urlencode("peoplereview/compose?ask=".$from->username); ?>" style="color: #01BF01;text-decoration:none;">
        <?php echo 'reviewing '.$from->user_detail->first_name." ".$from->user_detail->last_name; ?>
        </a>
        </p>
        </td>
        </tr>
