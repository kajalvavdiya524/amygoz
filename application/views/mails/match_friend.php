  ...
  <table class="sprty">
    <tr>
     ...
     <img alt="" src="https://m.callitme.com/upload/<?php echo $friend->photo->profile_pic;?>" class="img-rect posh-ytr" style="width: 50px;height: 50px;border-radius: 50%;float: left;margin-bottom: 39px;">
      <?php } else { ?>
       <span style="padding-left:10px;background-color:#bdbcbc;border-radius:50%;padding:16px;font-size:21px;margin:0px auto;text-align:center;float: left;margin-bottom: 39px;"> 
   ...
   <table>
        <tr>
      <td>
          <p style="color:#888; font-size:15px;padding: 11px 25px 4px 30px;">                                            
                <?php echo $friend->user_detail->get_name(); ?> is single.
                <br/>Connect <?php echo $friend->user_detail->get_name(); ?> with one of your friends.
          </p>
      </td>
    </tr> 
    </table>
  <table class="sprty">
    <tr class="row">
      <td class="col-xs-12" style="text-align: center;padding: 25px 0px 35px 0px;">                                             
        <a href="<?php echo url::base()."pages/redirect_to/".$user->username ."?page=".urlencode($friend->username); ?>" style="font-size:15px;color:#fff;text-decoration: none;background: #00bcd4;padding:9px 25px;">Match Friend
        </a>
      </td>
    </tr>
  </table>
