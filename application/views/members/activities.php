                ...
                foreach($activity_datas as $row) {
                    if(date('Y-m-d') != date('Y-m-d', strtotime($row['time']))) {
                        if($activity->id == $row['activity_id']) {?>
                            <div class="post activities_for_noti <?php if ($activity->time > $session_user->read_notification_at) { ?> unread <?php } ?>">
                                    ...
                                    <?php if ($activity->type != 'arequest') {
                                        ...
                                        <?php } else { ?>
                                            <a href="<?php echo url::base() . $activity->user->username; ?>">
                                                <?php $photo = $activity->user->photo->profile_pic_m;
                                                $activity_image = file_exists("mobile/upload/" .$photo);
                                                $activity_image1 = file_exists("upload/" .$photo);
                                                if (!empty($photo) && $activity_image) { ?>
                                                    <img src="<?php echo url::base() . "mobile/upload/" . $activity->user->photo->profile_pic_m; ?>" width="30" height="30">
                                                <?php }
                                                else if (!empty($photo) && $activity_image1) { ?>
                                ...
                                <div class="content pull-left hb-ml-10">
                                    <a href="<?php echo $href; ?>" class="<?php echo $class; ?>">
                                        ...
                                        <small class="post-time">
                                            ...
                                            <?php $age = time() - strtotime($activity->time);
                                            if ($age >= 86400) {
