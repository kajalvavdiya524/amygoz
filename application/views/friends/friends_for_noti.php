        ...
        <div class="post friend friends_for_noti hb-mb-5">
            ...
            <div class="user-img pull-left hb-mr-10">
                <a href="<?php echo url::base().$request->user->username; ?>">
                    <?php 
                    $photo = $request->user->photo->profile_pic_m;
                    $req_image = file_exists("mobile/upload/" .$photo);
                    $req_image1 = file_exists("upload/" .$photo);
                    if(!empty($photo) && $req_image) { ?>
                    <img src="<?php echo url::base()."mobile/upload/".$request->user->photo->profile_pic_m;?>" width="30" height="30">
                    <?php }
                    else if(!empty($photo) && $req_image1) { ?>
