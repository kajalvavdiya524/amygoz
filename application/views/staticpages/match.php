    ...
    <div class="container">
        <div class="row">
           ...
           <?php $top_users= DB::select('*',array('COUNT("to")', 'totel'))
                          ...
                          <?php 
                            $photo = $user_pic['profile_pic'];
                            $rec_image = file_exists("mobile/upload/" .$photo);
                            $rec_image1 = file_exists("upload/" .$photo);
                          if(!empty($photo) && $rec_image) { ?>
                            <img src="<?php echo url::base()."mobile/upload/".$user_pic['profile_pic'];?>" class="img-responsive" style="height: 100%; border-radius: 50%; width: 100%;">
                          <?php } 
                          if(!empty($photo) && $rec_image1) { ?>
